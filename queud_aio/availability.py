...
from queud_aio.config import AVAILABLE_SEAT_SYMBOL, BLOCK_MARKERS
...
def parse_event_page(html: str) -> tuple[str, bool, bool]:
    ...
    needs_login = (
        ...
        or (
            "Login" in title
            and "logged-in" not in html
            and "Event Information Screen" not in title
        )
    )
    if is_event_page(html):
        needs_login = False
