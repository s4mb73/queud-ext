...
def _env_bool(*keys: str, default: str = "0") -> bool:
    return _env(*keys, default=default).lower() in ("1", "true", "yes", "on")


DATA_DIR = Path(_env("QUEUD_AIO_DATA_DIR", "RUGBY_SA_DATA_DIR", default=str(PROJECT_ROOT / "data")))


def env_profiles_csv() -> str:
    return _env("QUEUD_AIO_PROFILES_CSV", "RUGBY_SA_PROFILES_CSV")


def env_profiles_row() -> str:
    return _env("QUEUD_AIO_PROFILE_ROW", "RUGBY_SA_PROFILE_ROW")
...
class Settings:
    ...
    def load(cls) -> Settings:
        ...
        max_price_raw = _env("QUEUD_AIO_CART_MAX_PRICE", "RUGBY_SA_CART_MAX_PRICE")
        ...
        return cls(
            ...
            cart_dry_run=_env_bool("QUEUD_AIO_CART_DRY_RUN", "RUGBY_SA_CART_DRY_RUN"),
            cart_max_price_zar=max_price,
            cart_allowed_sections=_parse_sections(
                _env("QUEUD_AIO_CART_SECTIONS", "RUGBY_SA_CART_SECTIONS")
            ),
            cart_retry_delay=float(os.environ.get("SPRINGBOKS_CART_DELAY_SEC", "0")),
            cart_max_attempts=int(os.environ.get("SPRINGBOKS_CART_MAX_ATTEMPTS", "2")),
            cart_lock_attempts=int(
                _env("QUEUD_AIO_CART_LOCK_ATTEMPTS", "RUGBY_SA_CART_LOCK_ATTEMPTS", default="2")
            ),
            ...
            tmpt_solver=os.environ.get("TMPT_SOLVER", "auto").lower(),
            tmpt_pool_workers=int(
                _env("QUEUD_AIO_TMPT_POOL_WORKERS", "RUGBY_SA_TMPT_POOL_WORKERS", default="3")
            ),
            tmpt_pool_cache_ttl=int(
                _env("QUEUD_AIO_TMPT_CACHE_TTL", "RUGBY_SA_TMPT_CACHE_TTL", default="240")
            ),
            tmpt_headless_timeout=int(
                _env(
                    "QUEUD_AIO_TMPT_HEADLESS_TIMEOUT",
                    "RUGBY_SA_TMPT_HEADLESS_TIMEOUT",
                    default="120",
                )
            ),
            tmpt_workers_dir=Path(
                _env(
                    "QUEUD_AIO_TMPT_WORKERS_DIR",
                    "RUGBY_SA_TMPT_WORKERS_DIR",
                    default=str(DATA_DIR / "tmpt_workers"),
                )
            ),
            tmpt_auto_fallback=_env_bool(
                "QUEUD_AIO_TMPT_AUTO_FALLBACK", "RUGBY_SA_TMPT_AUTO_FALLBACK", default="1"
            ),
            tmpt_headless_use_profile=_env_bool(
                "QUEUD_AIO_HEADLESS_USE_PROFILE", "RUGBY_SA_HEADLESS_USE_PROFILE"
            ),
            use_browser_requests=_env_bool(
                "QUEUD_AIO_BROWSER_REQUESTS", "RUGBY_SA_BROWSER_REQUESTS"
            ),
            session_max_steps=int(
                _env("QUEUD_AIO_SESSION_MAX_STEPS", "RUGBY_SA_SESSION_MAX_STEPS", default="12")
            ),
            queue_wait_seconds=int(
                _env("QUEUD_AIO_QUEUE_WAIT_SEC", "RUGBY_SA_QUEUE_WAIT_SEC", default="5")
            ),
