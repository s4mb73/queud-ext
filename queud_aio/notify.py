...
from queud_aio.models import EventSnapshot
...
def send_queud_checkout_discord(
    checkout_txt_path: Path,
    settings: Settings,
    *,
    data_dir: Path | None = None,
) -> None:
    """Send Adonis-style queud embed (Successful reserve + Proxy URL + fields)."""
    from queud_aio.discord_queud import send_queud_checkout_discord_message
    ...
    send_queud_checkout_discord_message(
        ...
        data_dir=data_dir or checkout_txt_path.parent,
    )
    ...
    *,
    ...
    if discord_webhook_url(settings):
        ...
        if snapshot.carted and checkout_file.exists():
            send_queud_checkout_discord(checkout_file, settings, data_dir=checkout_file.parent)
            sent = True
        elif not snapshot.carted:
            send_discord_embed(
