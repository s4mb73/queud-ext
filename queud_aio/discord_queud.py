...
from queud_aio.basket_meta import load_cart_meta
from queud_aio.queud import parse_queud_checkout_url, queud_discord_launcher_html
from pathlib import Path

from queud_aio.settings import Settings, DATA_DIR
...
def _discord_webhook(settings: Settings) -> str:
    from queud_aio.notify import discord_webhook_url
...
def build_launcher_click_urls(
    ...
    *,
    ...
    if settings.queud_api_base:
        from queud_aio.basket_api import register_checkout_pair
...
def build_queud_webhook_payload(
    ...
    *,
    ...
    saved = load_cart_meta(data_dir or DATA_DIR) or {}
...
def send_queud_checkout_discord_message(
    ...
    *,
    ...
    """Post queud checkout embed — one message, no file attachments."""
    webhook = _discord_webhook(settings)
    payload = build_queud_webhook_payload(
        settings,
        checkout_text,
        thumbnail_url=thumbnail_url,
        data_dir=data_dir,
