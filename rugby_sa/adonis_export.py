"""Export Adonis extension checkout files for manual testing."""

from __future__ import annotations

import time
from pathlib import Path

from rugby_sa.adonis import (
    adonis_launcher_html,
    build_adonis_reserve_urls,
    build_adonis_webhook_text,
    decode_adonis_payload,
)
from rugby_sa.browser_request import BrowserRequestClient
from rugby_sa.log_util import log
from rugby_sa.proxy import resolve_browser_proxy
from rugby_sa.settings import Settings


def export_adonis_checkout(settings: Settings) -> Path:
    """Write checkout.txt + adonis_proxy.html from live browser session."""
    out_dir = settings.http_session_file.parent
    out_dir.mkdir(parents=True, exist_ok=True)

    target = settings.event_targets[0]
    event_url = target.page_url(settings.base_url)
    basket_url = f"{settings.base_url}/Checkout/Basket"

    proxy_line = resolve_browser_proxy(settings)
    with BrowserRequestClient(settings, proxy_line=proxy_line) as client:
        client.ensure_event_page(event_url)
        log(f"Opening basket: {basket_url}")
        client.page.goto(basket_url, wait_until="domcontentloaded", timeout=120_000)
        time.sleep(2)
        raw = client._context.cookies() if client._context else []
        raw = [c for c in raw if c.get("value")]
        log(f"Collected {len(raw)} cookies for Adonis link")

        reserve_url, proxy_url = build_adonis_reserve_urls(
            raw, basket_url, proxy_line=client.proxy_line
        )

    # Validate payload decodes like the extension expects
    ext_b64 = proxy_url.split("extension=")[1].split("&endUrl=")[0]
    payload = decode_adonis_payload(ext_b64)
    cookie_count = len(payload.get("cookies") or [])
    log(f"Adonis payload OK — {cookie_count} cookies, keys={list(payload.keys())}")

    checkout_txt = out_dir / "checkout.txt"
    proxy_html = out_dir / "adonis_proxy.html"
    reserve_html = out_dir / "adonis_reserve.html"

    checkout_txt.write_text(
        build_adonis_webhook_text(
            reserve_url=reserve_url,
            proxy_url=proxy_url,
            store="Springboks TM Tickets",
            price="—",
            product=f"Event {target.event_id}",
            email=settings.sarugby_email or "—",
            quantity=settings.tickets_required,
            section="—",
            row="—",
            seat_start="—",
            seat_end="—",
        )
        + "\n",
        encoding="utf-8",
    )
    proxy_html.write_text(adonis_launcher_html(proxy_url), encoding="utf-8")
    reserve_html.write_text(adonis_launcher_html(reserve_url), encoding="utf-8")

    log("")
    log("=== Adonis extension checkout ===")
    log("1. Chrome → chrome://extensions → load AdonisExtension (unpacked)")
    log("2. Use a dedicated Chrome profile (not Incognito)")
    log(f"3. Double-click: {proxy_html}")
    log("   (uses Proxy URL — cookies + ZA proxy, then basket)")
    log(f"4. Or copy Proxy URL from: {checkout_txt}")
    log("   Paste in Chrome address bar (remove { } braces)")
    log(f"5. Reserve-only (no proxy): {reserve_html}")
    log("")

    return proxy_html