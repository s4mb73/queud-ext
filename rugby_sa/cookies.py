...
from rugby_sa.config import AKAMAI_COOKIE_NAMES, BLOCK_MARKERS
from rugby_sa.settings import Settings


def cookies_to_dict(session: curl_requests.Session) -> list[dict[str, str]]:
...
def load_cookies(session: curl_requests.Session, settings: Settings) -> None:
...
def has_cookie(session: curl_requests.Session, name: str, domain: str = "") -> bool:
    host = domain.lstrip(".")
    for cookie in session.cookies.jar:
        if cookie.name != name:
            continue
...
def save_cookies(session: curl_requests.Session, settings: Settings) -> None:
    path = settings.http_session_file
