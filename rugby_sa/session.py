...
from rugby_sa.auth import complete_sarugby_oauth, ensure_identity_tmpt
from rugby_sa.availability import is_event_page, parse_event_page
from rugby_sa.client import TmptClient
from rugby_sa.cookies import has_cookie, is_epsf_blocked
...
def has_browser_session(client: TmptClient, settings: Settings) -> bool:
    """Playwright-harvested Akamai cookies — do not HTTP-refresh tmpt."""
...
def ensure_page_access(
    ...
    if has_browser_session(client, settings) and is_epsf_blocked(resp.text):
        warn(
            "Browser session blocked or expired — re-run: python run.py --bootstrap"
        )
