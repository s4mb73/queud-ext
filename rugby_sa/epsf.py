...
def refresh_tmpt_http(client: Any, page_url: str) -> None:
    s = client.settings
    HttpTmptSolver(
        client.session, client.base_url, site_key=s.recaptcha_site_key
...
def refresh_tmpt(client: Any, page_url: str) -> None:
    ...
    if solver in ("headless", "headless-pool", "browser"):
        from rugby_sa.headless_tmpt import refresh_tmpt_headless
